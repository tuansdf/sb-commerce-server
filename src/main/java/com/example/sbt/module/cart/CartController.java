...
public class CartController {
    ...
    public ResponseEntity<CommonResponse<CartDTO>> save(@RequestBody CartDTO requestDTO) {
        try {
            var result = cartService.save(requestDTO);
    ...
    public ResponseEntity<CommonResponse<CartDTO>> findOne(@PathVariable UUID id) {
        try {
            var result = cartService.findOneByIdOrThrow(id);
            return ResponseEntity.ok(new CommonResponse<>(result));
        } catch (Exception e) {
            return ExceptionUtils.toResponseEntity(e);
        }
